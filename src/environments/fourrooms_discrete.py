...
class FourRoomsGridWorld(gym.Env):
    ...
    def __init__(
        ...
        start_noise=0.0,        # continuous noise around cell center
    ...
    def sample_initial_state(self):
        ...
        cell = self._free_cells[idx].astype(np.int64)
    ...
    def _action_to_delta(self, action):
        ACTION_TO_DELTA = {
            0: np.array([0, 1], dtype=np.int64),   # Up
            1: np.array([0, -1], dtype=np.int64),  # Down
            2: np.array([-1, 0], dtype=np.int64),  # Left
            3: np.array([1, 0], dtype=np.int64),   # Right
        }
        return self.step_scale * ACTION_TO_DELTA.get(action, np.array([0, 0], dtype=np.int64))
    ...
    def step(self, action):
        ...
        for _ in range(self.substeps):
            ...
            trial_x = np.array([trial[0], candidate[1]], dtype=np.int64)
            if self._is_valid_pos(trial_x):
                candidate[0] = trial_x[0]

            trial_y = np.array([candidate[0], trial[1]], dtype=np.int64)
    ...
    def valid_action_mask(self, obs):
        ...
        pos = np.asarray(obs, dtype=np.float32)

        # Convert position to current cell index
        cell = np.round(pos).astype(np.int32)
        x, y = int(cell[0]), int(cell[1])
        ...
        # Safety fallback: if somehow all are invalid, allow all
        if not mask.any():
            mask[:] = True
...
class FourRoomsGoalWrapper(gym.Wrapper):
    ...
    def _sample_slip_action(self):
        ...
        a = self.env.np_random.uniform(low=-1.0, high=1.0, size=(2,)).astype(np.int64)
        return a
